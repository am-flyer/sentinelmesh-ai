"""SSE streaming infrastructure for the Cybersecurity Threat Detection Agent API.
...
Agent display names (UI labels) map to coarse phases as follows:
Alert Intake -> alert_intake, Auth Analyzer -> auth_analysis,
Network Analyzer -> network_analysis, Threat Intel -> threat_intel,
Containment -> containment, SOC Reporter -> soc_reporting.
These correspond to the ``agent.name`` strings defined on each agent
(e.g. ``Alert Intake Agent`` maps to ``alert_intake``).
"""
...
AGENT_PHASE_MAP: dict[str, PhaseType] = {
    "Alert Intake Agent": "alert_intake",
    "Auth Analyzer Agent": "auth_analysis",
    "Network API Analyzer Agent": "network_analysis",
    "Threat Intel Agent": "threat_intel",
    "Containment Agent": "containment",
    "SOC Report Agent": "soc_reporting",
}


@dataclass
class RunState:
    """In-memory state for a single threat detection run.
    ...
    Attributes:
        ...
        report: Final SOC report text once completed.
    ...
    report: str = ""
    current_phase: PhaseType = "alert_intake"
...
def create_run(scenario_type: ScenarioType) -> RunState:
    ...
    Args:
        scenario_type: The threat scenario to simulate.
...
class StreamingThreatHooks(AgentHooks):
